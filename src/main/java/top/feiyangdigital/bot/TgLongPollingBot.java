    ...
    public void onUpdateReceived(Update update) {
        ...
        if (update.hasMessage() && update.getMessage().getChat().isUserChat()) {
            if (update.getMessage().getText().contains("start _")) {
                ...
                if (groupInfoWithBLOBs.getOwnerandanonymousadmins().contains(update.getMessage().getFrom().getId().toString())) {
                    ...
                    botHelper.sendInlineKeyboard(this, update);
                }
                ...
                try {
                    execute(sendContent.createResponseMessage(update, keywordsFormat, "html"));
                ...
                if ("allow".equals(addRuleCacheMap.getKeywordsFlagForUser(userId))) {
                    addAutoReplyRule.addNewRule(this, update);
                } else if ("candelete".equals(deleteRuleCacheMap.getDeleteKeywordFlagMap(userId))) {
                    deleteSingleRuleByKeyWord.DeleteOption(this, update);
                }
            }
        ...
        if (update.hasMessage() && (update.getMessage().getChat().isGroupChat() || update.getMessage().getChat().isSuperGroupChat())) {
            ...
            if (keywordsFormatList != null) {
                messageHandle.processUserMessage(this, update, keywordsFormatList);
            }
            if (("GroupAnonymousBot".equals(update.getMessage().getFrom().getUserName()) || checkUser.isChatOwner(this, update)) && ("/setbot".equals(update.getMessage().getText()) || ("/setbot@" + BaseInfo.getBotName()).equals(update.getMessage().getText()))) {
                GroupInfoWithBLOBs groupInfo = new GroupInfoWithBLOBs();
                groupInfo.setOwnerandanonymousadmins(adminList.fetchHighAdminList(this, update));
                ...
                botHelper.sendAdminButton(this, update);
            } else if ("/setbot".equals(update.getMessage().getText()) || ("/setbot@" + BaseInfo.getBotName()).equals(update.getMessage().getText())) {
                timerDelete.sendTimedMessage(this, sendContent.messageText(update, "你没有权限执行此命令"), 10);
            }
        }
        ...
        if (update.hasCallbackQuery()) {
            botHelper.handleCallbackQuery(this, update);
        }
    }
